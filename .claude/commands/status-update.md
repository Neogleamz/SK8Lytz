# /status-update — Situation Report (SITREP)

**Description:** Status Update Workflow — generate a SITREP dashboard showing current branch, epic progress, and next task.
**Persona:** 📋 Scrum — Casey
...
**Activated by:** `"whats up"`, `"we good"`, `"status update"`, `"where are we"`, `"what's happening"`, or `"current status"`

When my prompt includes any of the above, you must act as the Project Manager and execute the following workflow to generate a Situation Report (SITREP):

1. **Gather Git Context**:
   - Execute `git branch --show-current` to find the active branch.
   - Execute `git log -1 --pretty=%B` to get the most recent commit message.
   - Execute `git status --short` to see if there are uncommitted changes.
