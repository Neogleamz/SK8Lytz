...
2. **Validate and Merge:** Execute the automated gatekeeper script to verify the cryptographic attestation, confirm commit correlation and freshness, perform a fast-forward merge into master, dismantle the worktree, and prune the local feature branch:
   > **CRITICAL CWD REQUIREMENT:** You MUST set the working directory to exactly `C:\Neogleamz\AG_SK8Lytz_App\SK8Lytz`. Running this from inside the worktree directory will cause a silent failure loop.
...
1. **Build & Deploy:** Execute the `/deploy-device` workflow to compile the `master` code into a physical `.apk` and install it on the connected Android device.
