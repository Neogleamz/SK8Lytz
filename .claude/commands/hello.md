...
- **Any event at 3 occurrences** → IMMEDIATELY generate a Rule Evolution Proposal before proceeding.

**0.7b. KB Staleness Sweep:**
```powershell
node tools/kb-validator.js --summary
```
- **All current** → *"KB Health: CURRENT ✅. All [N] knowledge entries within staleness windows."*
- **⚠️ STALE entries** → List each with slug, days expired, domain.
- **🔴 CRITICAL entries** → Escalate: *"🔴 KB CRITICAL: [slug] — expired [N] days ago. Recommend /kb-refresh."*
...
5. **Bucket List Status**: Read `docs/SK8Lytz_Bucket_List.md` to determine the current project status.

6. Verify Database Backup Status
   - Use PowerShell to check the `backups/` directory for the most recent `.sql` file:
   ```powershell
   Get-ChildItem -Path C:\Neogleamz\AG_SK8Lytz_App\SK8Lytz\backups -Filter "data_*.sql" | Sort-Object LastWriteTime -Descending | Select-Object -First 1
   ```
