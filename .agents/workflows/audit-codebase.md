...
6. **Telemetry Sync**: Run `node tools/sync_remote_errors.mjs` to query the Supabase `telemetry_errors` table and print a triage summary. Flags: `--hours 48` (window), `--limit 50`, `--json` (raw output). Non-fatal if Supabase is offline.
