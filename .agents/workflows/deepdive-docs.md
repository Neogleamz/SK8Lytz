...
> 4. **ARCHITECTURAL IMPACT FLAGS**: You MUST append specific flags to your output if your domain's architectural surface changes:
...
> 5. **SEQUENCE DIAGRAM**: If your domain contains a complex multi-step process (e.g., Crew Creation, BLE Connection, OTA Update), you MUST generate a Mermaid Sequence Diagram (`sequenceDiagram`) detailing the exact actor-to-actor flow.
> 6. Write your final Markdown Payload to `artifacts/deepdive_docs/<DOMAIN_MARKER>_cartography.md` using the `write_to_file` tool. Do NOT use `send_message`.
...
### ⚡ Phase 4 — High-Level Documentation Synthesis

After the Master Reference is updated in Phase 3, Avery MUST execute Phase 4:
1. Read the flags generated by the 21 sub-agents in their `artifacts/deepdive_docs/` markdown files.
2. If `[IMPACTS_USER_JOURNEY]` is flagged, parse the changes and dynamically update the User Journey flows in `tools/User_Journey_Maps.md`.
3. If `[IMPACTS_C4_CONTEXT]` is flagged, update the Mermaid diagrams in `tools/System_Context_Diagram.md`.
4. If `[IMPACTS_STATE_CHART]` is flagged, update the state machines in `tools/State_Charts_UX.md`.
5. Finally, scan `tools/SESSION_LOG.md` for new `[DECISION]` entries related to these changes, and automatically sync them into `tools/Architecture_Decision_Records.md`.
6. Update the final workflow output summary to explicitly report which high-level documents were synced.
