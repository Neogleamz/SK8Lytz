...
3. **Branch Creation**:
   ...
   - **Safe-Commit Anchor (NEW)**: Execute `git commit -m "chore(checkpoint): pre-task anchor for <extracted-branch-slug>"` only if there are uncommitted changes to create a safe fallback point. This is MANDATORY for all tasks tagged `[H-RISK]`.
...
4. **Discovery & Clarification Phase (NEW)**:
   ...
   - **Devil's Advocate Gate**: If the task is tagged `[Feast]`, you must first run the **Devil's Advocate Workflow** (RULE[devils-advocate.md]) to identify 3 potential failure points before creating the plan.
...
5. **Planning & Mandatory Review Gate**:
   - Generate a detailed Implementation Plan using the `write_to_file` tool with `IsArtifact: true`.
   - **Crucial Branching:** If the task is tagged `[L-RISK]` AND (`[Snack]` OR `[Meal]`), you MUST physically read and strictly adhere to the `docs/plans/_TEMPLATE_flash_executable.md` format. This ensures your plan is a drift-tolerant, copy-paste ready script for a future execution model.
   - For `[H-RISK]` or `[Feast]` tasks, your plan MUST start with `### Design Decisions & Rationale` and explain your architectural choices in 2-3 sentences. Omit internal backtracking.
...
**Note: Stability-First Prioritization:** You must always organize the bucket list according to the following hierarchy, keeping the most critical items at the top and new feature integrations at the bottom:

1. **🔴 CRITICAL**: Performance, Stability & Security (Crashes, Data Corruption, RLS Blocks).
2. **🟠 HIGH**: Engineering Excellence & Tech Debt (Refactors, State Machines, Telemetry).
3. **🟡 MEDIUM**: Compliance & Governance (Legal, EULA, Admin Hub).
4. **🔵 LOW**: New Features & UI Enhancements (Product Value, Social, New Modes).
5. **❄️ ICEBOX**: Non-prioritized backburner items.

Always add items we are currently working on or have been suggested in conversations to the bucket list strictly following this order of priority. All changes from security, performance, or functionality reviews must be added to the appropriate section.

### 6. The Maintenance Sweep Protocol (Batching)

If the user requests a "Maintenance Sweep" or "Batch" of multiple small items:

1. **Scope Selection**: Autonomously filter the bucket list for items tagged `[BATCH]`. Group 3-5 items from the `CRITICAL`, `HIGH`, or `CHORE` categories.
2. **Branching**: Use a single branch named `chore/maintenance-sweep-[date]`.
3. **Collective Plan**: Generate one single Implementation Plan covering all items.
4. **Execution**: Implement all items together, verifying each individually during the test phase.

### 10. The Turbo & Autopilot Protocols (Velocity)
