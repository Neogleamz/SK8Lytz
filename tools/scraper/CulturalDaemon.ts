...
let internalEnrichedCount = 0;
let internalDenialCount = 0;
...
async function pushTelemetry(updates: { status: string; current_target?: string; last_error?: string | null }) {
  try {
    const { error } = await supabase.from('daemon_status').upsert({
      ...
      total_enriched: internalEnrichedCount,
      total_denials: internalDenialCount,
...
async function runEnrichmentCycle() {
  ...
  await pushTelemetry({ status: 'ONLINE', current_target: 'Fetching next payload...' });
  ...
  if (rpcError) {
    ...
    internalDenialCount++;
    await pushTelemetry({ status: 'ERROR', last_error: 'RPC Failed: ' + rpcError.message });
  ...
  if (!spots || spots.length === 0) {
    ...
    await pushTelemetry({ status: 'SLEEPING', current_target: 'Idle - Queue Empty' });
  ...
  await pushTelemetry({ status: 'ONLINE', current_target: `${targetSpot.name} (${targetSpot.city}, ${targetSpot.state})` });
  ...
  try {
    ...
    if (updateError) {
      ...
      internalDenialCount++;
      await pushTelemetry({ status: 'ERROR', current_target: targetSpot.name, last_error: 'Write Failed: ' + updateError.message });
    } else {
      console.log(`✅ Enrichment saved! Found Rating: ${culturalData.vibe_rating || 'N/A'}, Adult Night: ${culturalData.has_adult_night}`);
      internalEnrichedCount++;
    }
  } catch (e: any) {
    console.error(`❌ Complete failure during scrape:`, e.message);
    internalDenialCount++;
    await pushTelemetry({ status: 'ERROR', current_target: targetSpot.name, last_error: 'Scrape Crash: ' + e.message });
  }

  // 4. Sleep to prevent bans
  console.log(`😴 Cycle complete. Sleeping for ${SLEEP_INTERVAL_MS / 1000 / 60} minutes...`);
  await pushTelemetry({ status: 'SLEEPING', current_target: 'Cooldown Phase' });
...
async function startDaemon() {
  ...
  // Try to recover stats from DB on boot if restarting
  try {
    const { data } = await supabase.from('daemon_status').select('total_enriched, total_denials').eq('id', DAEMON_ID).single();
    if (data) {
      internalEnrichedCount = data.total_enriched || 0;
      internalDenialCount = data.total_denials || 0;
      console.log(`🔄 Recovered Telemetry: Enriched [${internalEnrichedCount}] | Denials [${internalDenialCount}]`);
    }
  } catch(e) {}
