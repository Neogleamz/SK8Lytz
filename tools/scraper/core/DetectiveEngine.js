...
exports.safeSurface = exports.safeBool = exports.safeNum = void 0;
exports.executeDetective = executeDetective;
...
const safeBool = (v) => {
    ...
    if (typeof v === 'string') {
        ...
        // Exact matches first
        if (['yes', 'true', '1', 'y', 'available', 'offered'].includes(lower))
            return true;
        if (['no', 'false', '0', 'n', 'none'].includes(lower))
            return false;
        if (['null', 'unknown', 'not available', 'n/a', 'not mentioned', 'not specified'].includes(lower))
            return null;
...
exports.safeSurface = safeSurface;
// ─── Constants & Utilities ───────────────────────────────────────────────────
...
function isSocialCrawlBlocked(url) {
    try {
 ...
 */
async function executeDetective(spotContext, aiConfig, isHeadless, onProgress = () => { }) {
    let browser = null;
    let combinedText = '';
    const allLinks = [];
    let ogImage = null;
                ...
                while (passCount <= 2) {
                    ...
                    while (crawlQueue.length > 0 && combinedText.length < 50000) {
                        ...
                        if (isDirectImageUrl) {
                            ...
                            try {
                                const { data: { text } } = await tesseract_js_1.default.recognize(url, 'eng');
                        ...
                        const pageData = await page.evaluate((isPriority) => {
                            ...
                            const iframes = Array.from(document.querySelectorAll('iframe'))
                                .map((el) => el.src || '')
                                .filter((src) => src.includes('calendar') || src.includes('ticket') || src.includes('centeredge') || src.includes('roller'));
                        ...
                        const pageImages = pageData.imgs.map((i) => i.src);
                        domImages.push(...pageImages.slice(0, 2));
                        const trappedFlyers = pageData.imgs.filter((i) => i.isFlyer).map((i) => i.src);
                    ...
                    let hostname = '';
                    try {
                        hostname = new URL(spotContext.website).hostname;
                    }
                    catch {
                        hostname = '';
                    }
                    ...
                    for (const link of internalLinks) {
                        ...
                        if (bestScore > 0)
                            scored.push({ href: link.href, score: bestScore });
                    }
                    ...
                    for (const url of subUrls) {
                        ...
                        if (combinedText.includes(`[PAGE: ${url}]`))
                            continue;
                        ...
                        try {
                            ...
                            const subData = await page.evaluate(() => {
                                ...
                                const jsonLd = Array.from(document.querySelectorAll('script[type="application/ld+json"]')).map((el) => el.innerText).join('\n');
                                const iframes = Array.from(document.querySelectorAll('iframe'))
                                    .map((el) => el.src || '')
                                    .filter((src) => src.includes('calendar') || src.includes('ticket') || src.includes('centeredge') || src.includes('roller'));
                                const flyers = Array.from(document.querySelectorAll('img')).filter((img) => {
                                    const w = img.naturalWidth || img.width || 0;
                                    const h = img.naturalHeight || img.height || 0;
                            ...
                            if (subData.jsonLd)
                                combinedText += `\n\n[HIDDEN JSON-LD SCHEMA: ${url}]\n${subData.jsonLd}`;
                            if (subData.cleanText)
                                combinedText += `\n\n[PAGE: ${url}]\n${subData.cleanText}`;
                            if (subData.iframes.length > 0)
                                crawlQueue.push(...subData.iframes);
                            if (subData.flyers.length > 0)
                                flyerUrls.push(...subData.flyers);
                        }
                        catch {
                    ...
                    if (flyerUrls.length > 0) {
                        ...
                        for (const fUrl of uniqueFlyers) {
                            ...
                            if (combinedText.includes(`[OCR from Flyer Image: ${fUrl}]`))
                                continue;
                            try {
                                const { data: { text } } = await tesseract_js_1.default.recognize(fUrl, 'eng');
                    ...
                    const userSchema = userVectors.reduce((acc, vec) => {
    ...
    const toxicityReason = exclusions.find((kw) => combinedText.toLowerCase().includes(kw.toLowerCase()));
    if (toxicityReason) {
    ...
    const surface_type = (0, exports.safeSurface)(aiMetadata.surface_type || spotContext.surface_type);
    const surface_quality = aiMetadata.surface_quality || spotContext.surface_quality || null;
    const vibe_score = (0, exports.safeNum)(aiMetadata.vibe_score ?? spotContext.vibe_score);
    const is_indoor = (0, exports.safeBool)(aiMetadata.is_indoor ?? spotContext.is_indoor);
    const has_fee = (0, exports.safeBool)(aiMetadata.has_fee ?? spotContext.has_fee);
    const has_adult_night = (0, exports.safeBool)(aiMetadata.has_adult_night ?? spotContext.has_adult_night) ?? false;
    ...
    const has_rental = (0, exports.safeBool)(aiMetadata.has_rental ?? spotContext.has_rental);
    const has_pro_shop = (0, exports.safeBool)(aiMetadata.has_pro_shop ?? spotContext.has_pro_shop);
    const has_food = (0, exports.safeBool)(aiMetadata.has_food ?? spotContext.has_food);
    const has_lights = (0, exports.safeBool)(aiMetadata.has_lights ?? spotContext.has_lights);
    const has_lockers = (0, exports.safeBool)(aiMetadata.has_lockers ?? spotContext.has_lockers);
    const has_ac = (0, exports.safeBool)(aiMetadata.has_ac ?? spotContext.has_ac);
    const has_wifi = (0, exports.safeBool)(aiMetadata.has_wifi ?? spotContext.has_wifi);
    const has_toilets = (0, exports.safeBool)(aiMetadata.has_toilets ?? spotContext.has_toilets);
    const is_wheelchair_accessible = (0, exports.safeBool)(aiMetadata.wheelchair ?? aiMetadata.is_wheelchair_accessible ?? spotContext.is_wheelchair_accessible);
    const hosts_derby = (0, exports.safeBool)(aiMetadata.derby ?? aiMetadata.hosts_derby ?? spotContext.hosts_derby);
    ...
    // ── Photo Candidates ─────────────────────────────────────────────────────
    let candidatePhotos = spotContext.candidate_photos || null;
    if (!spotContext.photos && !candidatePhotos) {
        const candidateMap = {};
        if (ogImage)
            candidateMap.og_image = ogImage;
        if (domImages.length > 0)
            candidateMap.dom_images = [...new Set(domImages)].slice(0, 3);
