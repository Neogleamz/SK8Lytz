...
- [ ] `fix/rls-telemetry-block` : [CLOUD] [BATCH] [H-RISK] [Snack] [CRITICAL] Resolve 403 Forbidden errors on `parsed_session_stats`, `device_diagnostics`, and `parsed_session_devices`. Likely RLS policy mismatch for authenticated telemetry sinks. → [Plan](docs/plans/fix-rls-telemetry-block.md)
...
- [ ] `chore/telemetry-standards` : [CLOUD] [L-RISK] [Meal] Implement the 5-point 'Black Box' logging standard (JSON, HW Context, PII Masking, Levels, FIFO Buffer). → [Plan](docs/plans/chore-telemetry-standards.md)
