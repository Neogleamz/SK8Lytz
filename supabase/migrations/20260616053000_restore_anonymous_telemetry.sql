-- Restore Anonymous Telemetry Ingestion

-- 1. Enable `anon` Role Inserts (RLS Bypass for Telemetry)
DROP POLICY IF EXISTS "Enable anon insert for telemetry_snapshots" ON public.telemetry_snapshots;
CREATE POLICY "Enable anon insert for telemetry_snapshots"
ON public.telemetry_snapshots FOR INSERT
TO anon
WITH CHECK (true);

DROP POLICY IF EXISTS "Enable anon insert for crash_telemetry" ON public.crash_telemetry;
CREATE POLICY "Enable anon insert for crash_telemetry"
ON public.crash_telemetry FOR INSERT
TO anon
WITH CHECK (user_id IS NULL);

DROP POLICY IF EXISTS "Enable anon insert for telemetry_errors" ON public.telemetry_errors;
CREATE POLICY "Enable anon insert for telemetry_errors"
ON public.telemetry_errors FOR INSERT
TO anon
WITH CHECK (true);

-- 2. Update the flush_telemetry function to route anonymous performance dumps to telemetry_snapshots
...
BEGIN
    ...
    IF v_user_id IS NULL THEN
        -- Route anonymous performance telemetry into telemetry_snapshots
        INSERT INTO public.telemetry_snapshots (session_id, event_type, metadata)
        VALUES ('anonymous_session', 'anonymous_performance_dump', payload);
        RETURN;
    END IF;
