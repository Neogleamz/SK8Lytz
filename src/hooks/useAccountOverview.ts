...
export function useAccountOverview(visible: boolean, onProfileUpdated?: () => void) {
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      ...
      AppLogger.error('[AccountOverview] handleSaveProfile failed', { error: e instanceof Error ? e.message : String(e)  , payload_size: 0, ssi: 0 });
  ...
  const handlePickProfilePhoto = async () => {
    ...
    try {
      ...
      AppLogger.error('[AccountOverview] handlePickProfilePhoto failed', { error: e instanceof Error ? e.message : String(e)  , payload_size: 0, ssi: 0 });
  ...
  const handleLeaveCrew = async (crewId: string) => {
    ...
    try {
      ...
      AppLogger.error('[AccountOverview] handleLeaveCrew failed', { crewId, error: e instanceof Error ? e.message : String(e)  , payload_size: 0, ssi: 0 });
