...
describe('registerProtocol', () => {
  it('does not register a duplicate protocol ID', () => {
    ...
    const fakeZengge: IControllerProtocol = {
      ...
      buildSetRfRemoteState: () => ({ packets: [], interPacketDelayMs: 0, isRateLimited: false }),
      buildClearRfRemotes: () => ({ packets: [], interPacketDelayMs: 0, isRateLimited: false }),
      buildWriteSettingsByName: () => ({ packets: [], interPacketDelayMs: 0, isRateLimited: false }),
