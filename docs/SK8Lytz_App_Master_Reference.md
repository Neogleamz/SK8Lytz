...
| **Scheduled session insert behavior** | `createSession` (Manager): if `opts.scheduledAt` is set, explicitly inserts `is_active: false` (prevents early joins — DB default was unconfirmed). Immediate sessions insert `is_active: true`. Server-side cron (`activate-scheduled-crews`) is the sole mechanism that flips `status='scheduled'` → `status='active', is_active=true`. | Do NOT flip `is_active` on the client for scheduled rows. |
| **Scheduled-join error path** | `joinSession` (Manager): if the primary query (`.eq('is_active', true)`) fails, a secondary `.maybeSingle()` query checks for a `status='scheduled'` row with the same invite code. If found: throws `"This crew session has not started yet. You will be notified when it goes live."` instead of the generic "not found" error. `joinSessionById` is UNCHANGED (activation cron is the correct gate). | Do NOT remove or relax the `is_active=true` filter on `joinSession` or `joinSessionById`. |
...
| `activate-scheduled-crews` | `activate-scheduled-crews/index.ts` | Deno Edge Function. **Cron-invoked only** (NOT user-facing). Auth via shared `CRON_SECRET` header (`x-cron-secret`). Every minute: (1) selects `crew_sessions` where `status='scheduled' AND scheduled_at <= now()`, (2) flips them to `status='active', is_active=true` via service-role client (idempotent — guarded by `.eq('status','scheduled')`), (3) fetches `push_tokens` joined to `crew_memberships` for each activated session's crew, (4) sends "🛼 Crew is Live!" Expo push in batches of 100. Push data payload: `{ crewId, sessionId }` — tapped by `NotificationService._wireResponseHandler` which calls `joinHandler(crewId, sessionId)`. Deployment requires migration `20260622000000_activate_scheduled_crews_cron.sql`. See PLAN-feat-crew-scheduled-server-side.md. |
...
| `20260622000000_activate_scheduled_crews_cron.sql` | **DEPLOY-TIME PLACEHOLDERS**: `<PROJECT_REF>` and `<CRON_SECRET>` must be substituted before `supabase db push`. Enables `pg_cron` + `pg_net` extensions (idempotent). Stores the edge function URL and cron secret in Supabase Vault (`activate_scheduled_crews_url`, `activate_scheduled_crews_secret`). Schedules a `cron.schedule` job (`activate-scheduled-crews-every-min`, `* * * * *`) that calls `pg_net.http_post` to the `activate-scheduled-crews` edge function with the secret header. Rollback: `cron.unschedule('activate-scheduled-crews-every-min')` + `vault.delete_secret(...)`. |
