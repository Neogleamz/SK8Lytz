...
- *Note:* The actual `Sk8LytzProgrammerModal` component and `isProgrammerVisible` state will remain in the file because it is still triggered by the Device Settings modal (`onOpenProgrammer()`), preserving the underlying functionality while just removing the redundant visual clutter from the dashboard header.
