...
We are using client-side derivation based on standard `Location.watchPositionAsync` coordinate deltas mapped to standard imperial measurements (`mph`, `miles`) to achieve sub-second live telemetry in `DockedController`, combined with isolated `useCrewStore` writes. This completely avoids network spam during tracking, accumulating purely locally until an explicit "End Session" action pushes a single batch sync up to Supabase. This fulfills our "low latency, low data footprint" constraint.
