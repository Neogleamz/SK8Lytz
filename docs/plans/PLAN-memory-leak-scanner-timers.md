...
Both timer cleanups (`clearTimeout(debounceTimerRef.current)` and `clearTimeout(telemetryTimerRef.current)`)
were already present in `src/hooks/ble/useBLEScanner.ts` (lines 411–415) as of commit `c795df69`
