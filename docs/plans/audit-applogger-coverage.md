...
While we track the raw BLE payloads sent to the device, we do not know what the user _specifically tapped_ in the application.
...
> [!IMPORTANT]
> Is there a specific threshold for telemetry ingestion you want to enforce? For example, should we log _every single drag_ on the Speed Slider, or only log it `onSlidingComplete` to prevent flooding the database with hundreds of packets per second?
