...
- **Group-First Hierarchy:** The `Devices` tab inside `AccountModal` will be completely overhauled. Instead of a flat list, devices will be aggregated by their `group_name`. We will render high-level Group Cards that allow users to rename or forget the *entire group* at once, with child elements for individual devices underneath them.
