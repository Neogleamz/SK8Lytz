...
2. **Voice Mic Access:** Mobile platforms require explicit user consent. We will implement `react-native-permissions` or standard `PermissionsAndroid`/`expo-av` permission calls in `useVoiceControl.ts` to request and verify mic access *before* calling `Voice.start()`, avoiding silent initialization failures and app crashes.
