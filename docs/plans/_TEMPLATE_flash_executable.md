...
*   [ ] **Check 1:** Open `src/path/to/file.tsx`. Search for semantic anchor: `export const TargetFunction = () => {`. 
    *   *Expected state:* The function should exist and currently take X arguments.
    *   *Abort Condition:* If the function has been renamed or heavily refactored into a hook, **HALT** and instruct the user: *"Codebase has drifted. This plan is stale and must be recompiled by a THINK model."*

*(Add as many pre-flight checks as necessary to ensure the environment is safe).*

---

## 2. Step-by-Step Execution Strict Instructions

*(Break the task down into atomic, copy-paste ready tool operations).*

### Step 2.1: [Action Description]
- **Target File:** `src/path/to/target.tsx`
- **Semantic Anchor / Target Content:** 
  *(Describe exactly what the executing model should search for using `view_file` or `grep_search` before running `replace_file_content`. e.g., "The entire `return` block of the `renderItem` function").*
...
*(Provide exact terminal commands and expected output strings to prove the execution worked).*
