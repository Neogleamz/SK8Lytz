...
> [!TIP]
> Good news: Our native codebase (`App.tsx` and `AppLogger.ts`) _already_ hooks into `global.ErrorUtils` and instances a `SafeErrorBoundary`. Any fatal JS breakdown or React component crash is already silently logged to `parsed_logs` as `ERROR_CAUGHT` via our Supabase backend. We don't need Sentry!
