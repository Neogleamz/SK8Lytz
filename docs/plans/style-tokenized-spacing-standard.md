...
*   [ ] **Check 1:** Open `src/theme/theme.ts`. Search for semantic anchor: `export const Layout = {`. 
    *   *Expected state:* The `Layout.padding` property should exist and be set to `14` or another numeric literal.
    *   *Abort Condition:* If `Spacing` is already exported, **HALT** and instruct the user: *"Codebase has drifted. This plan is stale and must be recompiled by a THINK model."*
...
function processFiles(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      const regex = /(margin|padding|gap)(Top|Bottom|Left|Right|Horizontal|Vertical)?:\s*([0-9]+)\b/g;
      ...
      if (modified) {
        ...
        const themeRegex = /import\s+{([^}]*)}\s+from\s+['"]([^'"]*theme\/theme)['"]/;
        const themeMatch = content.match(themeRegex);

        if (themeMatch) {
          if (!themeMatch[1].includes('Spacing')) {
            content = content.replace(themeRegex, `import { ${themeMatch[1].trim()}, Spacing } from '${themeMatch[2]}'`);
          }
        } else {
          // If no existing theme import, insert a new relative import block at the end of the existing imports
          let relPath = path.relative(path.dirname(fullPath), THEME_PATH).replace(/\\\\/g, '/').replace('.ts', '');
        ...
        console.log(`Replaced ${matchCount} usages in ${path.basename(fullPath)}`);
      }
    }
  }
}
...
*   [ ] **Command:** `npx tsc --noEmit`
    *   *Expected Output:* The compiler should cleanly parse all files with no syntax errors related to the injected `Spacing` variable. Note: Pre-existing TS errors (`DashboardScreen` type mismatches) are acceptable and do not warrant a rollback.
*   [ ] **Command:** `git diff`
