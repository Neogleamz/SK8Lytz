...
- Instead of only parsing "connectable" or "claimed" devices, `useBLE` will log the full MAC address, RSSI, and payload data of _every_ detected SK8Lytz-compatible board.
...
- It will explicitly tag whether a device is _Registered to You_, _Unregistered_, or _Claimed by Someone Else_, mirroring the data synced to Supabase.
